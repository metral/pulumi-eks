...
export function createCore(name: string, args: ClusterOptions, parent: pulumi.ComponentResource): CoreData {
    ...
    if (typeof storageClasses === "string") {
        ...
        userStorageClasses.apply(usc => {
            usc[storageClasses] = createStorageClass(`${name.toLowerCase()}-${storageClasses}`, storageClass, { parent: parent, provider: provider });
        });
    } else {
        for (const key of Object.keys(storageClasses)) {
            userStorageClasses.apply(usc => {
                usc[key] = createStorageClass(`${name.toLowerCase()}-${key}`, storageClasses[key], { parent: parent, provider: provider });
            });
        }
    }
