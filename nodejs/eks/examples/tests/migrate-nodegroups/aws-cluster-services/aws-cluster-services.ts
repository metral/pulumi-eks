...
export function create(
    ...
    const fluentdCloudWatchRole = newRoleWithPolicies(
        "kubeIamRoleFluentdCloudWatch",
        {
            ...
            assumeRolePolicy: pulumi.all([
                args.instanceRoleArn,
            ]).apply(([roleArn]) => {
                let a = pulumi.output(assumeUserRolePolicy(roleArn));
                return a.json;
            }),
        },
        {
    ...
    const fluentdCloudWatchName = "fluentd-cloudwatch";
    const fluentdCloudWatch = new fluentd.FluentdCloudWatch(fluentdCloudWatchName, {
        clusterName: args.clusterName,
        image: "fluent/fluentd-kubernetes-daemonset:v1.3.3-debian-cloudwatch-1.4",
        labels: {app: fluentdCloudWatchName},
        region: args.region,
        namespace: args.namespace,
